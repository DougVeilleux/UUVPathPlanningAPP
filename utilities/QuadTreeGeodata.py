...
class QuadTree:
    ...
    def find_closest_node(self, point, initial_search_radius=0.001, search_radius_increment=0.005, max_search_radius=0.05):
        """
        ...
        :param initial_search_radius: the initial radius within which to search for the closest node (decimal degrees)
        :param search_radius_increment: the amount by which the search radius is incremented if no node is found (decimal degrees)
        :param max_search_radius: the maximum search radius allowed (decimal degrees)
        :return: closest node within the search radius, nodes within the search region, and the final search region
        """
        ...
        while search_radius <= max_search_radius:
            ...
            # If no nodes are found within the current search radius, increment the radius
            search_radius += search_radius_increment

        # If no node is found within the maximum search radius, return None for all values
